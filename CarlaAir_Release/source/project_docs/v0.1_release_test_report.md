...
- 工作空间：自建的 `carla_ros_ws/`，13 个包，构建耗时 25.6s
- 需要 5 个兼容性补丁：
...
- 使用预构建：AirSim 1.7.0 的 ROS2 wrapper（`AirSim-1.7.0-linux/ros2/`）
...
2. **CARLA ROS Bridge 兼容性**：`carla-ros-bridge` (v0.9.13) 需要 5 个补丁才能与 CARLA 0.9.16 + ROS2 Humble 兼容。补丁已在 `carla_ros_ws/src/ros-bridge/` 中应用。
...
| `carla_ros_ws/` | T2 CARLA ROS Bridge 工作空间（用户自行构建） |
