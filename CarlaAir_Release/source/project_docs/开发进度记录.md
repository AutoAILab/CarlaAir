...
|------|------|
| CARLA 源码根目录 | 本仓库根目录 |
| UE4 4.26 引擎 | `$UE4_ROOT`（用户自行设置） |
| CARLA Content | `Unreal/CarlaUE4/Content/Carla/` |
| 独立版 CARLA | （原版 CARLA，用于对比测试） |
| AirSim 配置文件 | `~/Documents/AirSim/settings.json` |
| Conda 环境 | `simworld`（已安装 airsim 和 carla 包） |
| 一键启动脚本 | `./carlaAir.sh`（仓库根目录） |
| 示例脚本目录 | `./examples/` |
| 本文档 | `CarlaAir_Release/source/project_docs/开发进度记录.md` |
| 测试教程 | `CarlaAir_Release/source/project_docs/测试教程.md` |
...
**文件路径：** `~/Documents/AirSim/settings.json`
...
```bash
export UE4_ROOT=<path_to_your_ue4_4.26>
cd <path_to_carla_source>   # 本仓库根目录
...
**输出**：`Dist/CARLA_Shipping_*/LinuxNoEditor/`（19GB）
...
1. **对比测试**：原版 CARLA加载 Town10HD_Opt，车辆 z≈0.0 正常；CarlaAir 加载 Town10HD，车辆 z≈-29.6 穿地
...
完整测试报告：`CarlaAir_Release/source/project_docs/v0.1_release_test_report.md`
