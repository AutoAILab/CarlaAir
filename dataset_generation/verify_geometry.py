...
def verify_frame_geometry(data_dir, frame_id=0):
    logging.info(f"=== Geometric Validation: Frame {frame_id} ===")
    
    # 1. Load Metadata
    meta_path = os.path.join(data_dir, 'metadata', f'{frame_id:06d}.json')
    if not os.path.exists(meta_path):
        logging.error(f"Metadata not found at {meta_path}")
        return
    ...
    logging.info("[Stage 1] Semantic Absolute Alignment...")
    if uav_names:
        ...
        if os.path.exists(uav_gb_path):
            ...
            if np.any(vehicle_mask):
                ...
                logging.debug(f"  UGV Detection Centroid @ {veh_centroid}")
                logging.debug(f"  UGV Ground Truth @ {ugv_pos}")
                logging.info(f"  Reprojection Distance: {dist:.4f}m")
                if dist < 10.0:
                    logging.info("  PASS: High-confidence world alignment verified.")
                else:
                    logging.error("  FAIL: Semantic centroid mismatch.")
            else:
                logging.info("  SKIPPED: UGV not found in UAV view.")

    # --- STAGE 2: Corrected Project-Back Consistency ---
    logging.info("[Stage 2] Sub-pixel Project-Back Check...")
    if len(uav_names) >= 2:
        uav1_name = uav_names[0]
        uav2_name = uav_names[1]
        u1_gb_path = os.path.join(data_dir, 'gbuffer', f'{uav1_name}_{frame_id:06d}.npz')
        u2_gb_path = os.path.join(data_dir, 'gbuffer', f'{uav2_name}_{frame_id:06d}.npz')
        
        if os.path.exists(u1_gb_path) and os.path.exists(u2_gb_path):
            u1_gb = np.load(u1_gb_path)
            u2_gb = np.load(u2_gb_path)
            ...
            if np.any(valid):
                ...
                if p_local_u2[0] > 0:
                    ...
                    if 0 <= u_px < w2 and 0 <= v_px < h2:
                        ...
                        logging.info(f"  Reprojection Error: {err:.4f}m")
                        if err < 1.0:
                            logging.info("  PASS: Multi-view geometric locking confirmed.")
                        else:
                            logging.error("  FAIL: Parallax drift detected.")

    # --- STAGE 3: Raw Lidar Alignment (UGV) ---
    logging.info("[Stage 3] Raw Lidar Alignment...")
        ...
        if os.path.exists(ugv_gb_path):
            ...
            if depth_linear is not None:
                ...
                if len(p_world_depth) > 0:
                    ...
                    logging.info(f"  Cloud-to-Lidar Mean Offset: {d_mean:.4f}m")
                    if d_mean < 0.1:
                        logging.info("  PASS: Depth-to-Lidar alignment verified.")
                    else:
                        logging.error("  FAIL: Significant Depth-to-Lidar drift.")
