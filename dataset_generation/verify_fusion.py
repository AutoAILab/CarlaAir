...
def verify_fusion(data_dir, frame_id=0):
    logging.info(f"=== Fusion Validation: Frame {frame_id} ===")
    ...
    if os.path.exists(ugv_gb_path):
        ...
        if 'depth_linear' in ugv_gb:
            ...
            logging.info(f"  Added {len(p_world)} points from UGV.")

    # 2. Reproject UAVs
    for uav_name in meta['uavs'].keys():
        logging.info(f"Reprojecting {uav_name}...")
        ...
        if os.path.exists(uav_gb_path):
            ...
            master_cloud.append(p_world)
            logging.info(f"  Added {len(p_world)} points from {uav_name}.")
    ...
    if master_cloud:
        ...
        logging.info(f"Fusion result saved to: {out_path}")
        logging.info(f"Total points: {len(full_cloud)}")
        
        bbox_min = np.min(full_cloud, axis=0)
        bbox_max = np.max(full_cloud, axis=0)
        diag = np.linalg.norm(bbox_max - bbox_min)
        logging.info(f"Cloud Diagonal: {diag:.2f}m")
        if diag < 500:
             logging.info("PASS: Fusion consistency verified.")
        else:
             logging.warning("FAIL: Fusion diagonal too large, potential alignment error.")
    else:
        logging.error("FAIL: No data to fuse.")
